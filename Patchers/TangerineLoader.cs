...
namespace Tangerine.Patchers
{
    ...
    public class TangerineLoader
    {
        ...
        internal static readonly ModDictionary<string, HashSet<string>> AssetBundleDepends = new();
        ...
        /// <summary>
        /// Adds a list of dependent Asset Bundles to get loaded with the linked bundle
        /// </summary>
        /// <param name="name">Name of the <see cref="AssetbundleId"/> to add dependencies that need loaded</param>
        /// <param name="dependencies">List of dependent bundles to load</param>
        public void AddAssetBundleDependencies(string name, HashSet<string> dependencies)
        {
            AssetBundleDepends.Set(_modGuid, name, dependencies);
        }

        /// <summary>
        /// Removes a file mapping that was added before
        /// </summary>
        ...
        private static void GetBundleDependencies_Postfix(AssetsBundleManager __instance, string bundleName, AssetKeepMode keepMode)
        {
            if (AssetBundleDepends.Base.TryGetValue(bundleName, out var dependencies))
            {
                foreach (var bun in dependencies)
                    __instance.StartCoroutine(__instance.OnStartLoadSingleAsset(bun, AssetKeepMode.KEEP_ALWAYS));
