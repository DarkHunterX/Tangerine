...
namespace Tangerine.Patchers.DataProvider
{
    ...
    public class TangerineDataManager
    {
        ...
        /// <summary>
        /// Removes a table entry from <see cref="OrangeDataManager"/> that was added before.
        /// Will restore original entry if it exists.
        /// </summary>
        /// <param name="key">Key of the table entry to remove</param>
        /// <param name="dataTableDictName">Name of the table dictionary property in <see cref="OrangeDataManager"/></param>
        public void UnpatchTable(int key, string dataTableDictName)
        {
            var patchDict = Provider.GetPatchDict(dataTableDictName);
            patchDict.Remove(_modGuid, key);
        }
